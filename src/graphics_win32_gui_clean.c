...
static void calculate_layout(GraphicsWin32GUI* gfx, Layout* layout) {
    ...
    // Zone console (gauche) : hauteur - boutons
    layout->console_h = layout->window_h - BUTTON_HEIGHT;
    ...
    // LCD : positions relatives dans la zone console
    layout->lcd_x = layout->console_w * 0.24;
    layout->lcd_y = layout->console_h * 0.21;
    layout->lcd_w = layout->console_w * (0.76 - 0.24);
    layout->lcd_h = layout->console_h * (0.68 - 0.21);
    
    // Panneaux (droite) : tout l'espace restant
    layout->panels_x = layout->console_w;
    layout->panels_w = layout->window_w - layout->console_w;
    layout->panels_h = layout->console_h;
    
    // Boutons : en bas de la zone console
    layout->buttons_y = layout->console_h;
}
    ...
    switch (uMsg) {
        ...
        case WM_PAINT: {
            ...
            if (gfx) {
                ...
                // 3. Dessiner l'écran LCD (PRIORITÉ ABSOLUE - DERNIER)
                if (gfx->framebuffer) {
                    SetStretchBltMode(hdc, STRETCH_HALFTONE);
                    
                    // Rendu direct du framebuffer via StretchDIBits
                    StretchDIBits(
                        hdc,
                        layout.lcd_x,
                        layout.lcd_y,
        ...
        case WM_SIZE: {
            ...
            if (gfx->hSerial) {
                int serial_h = layout.panels_h * 2 / 3;
                SetWindowPos(gfx->hSerial, HWND_TOP, layout.panels_x, 0, 
                           layout.panels_w, serial_h, SWP_SHOWWINDOW);
            }
            
            if (gfx->hLogs) {
                int logs_y = layout.panels_h * 2 / 3;
                int logs_h = layout.panels_h - logs_y;
                SetWindowPos(gfx->hLogs, HWND_TOP, layout.panels_x, logs_y, 
                           layout.panels_w, logs_h, SWP_SHOWWINDOW);
            }
            
            // Repositionner les boutons
            int button_start_x = (layout.console_w - (8 * BUTTON_WIDTH + 7 * BUTTON_SPACING)) / 2;
            ...
            for (int i = 0; i < 8; i++) {
                if (buttons[i]) {
                    SetWindowPos(buttons[i], HWND_TOP, button_start_x, layout.buttons_y,
...
bool graphics_win32_gui_init(GraphicsWin32GUI* gfx) {
    ...
    gfx->hBackground = (HBITMAP)LoadImageA(NULL, "gameboy_bg.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
    if (!gfx->hBackground) {
        gfx->hBackground = (HBITMAP)LoadImageA(NULL, "gameboy_bg.jpg", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
    }
    ...
    if (gfx->hBackground) {
        ...
        printf("Image de fond chargee: %dx%d\n", gfx->bg_width, gfx->bg_height);
    } else {
        printf("Impossible de charger l'image de fond, utilisation d'un fond blanc\n");
        gfx->bg_width = 800;
        gfx->bg_height = 600;
    }
    ...
    int win_w = 1200; // Largeur généreuse pour les panneaux
    ...
    gfx->hSerial = CreateWindowEx(
        ...
        layout.panels_x, 0, layout.panels_w, layout.panels_h * 2 / 3,
        gfx->hwnd,
        (HMENU)2000,
        GetModuleHandle(NULL),
        NULL
    ...
    gfx->hLogs = CreateWindowEx(
        ...
        "",
        WS_CHILD | WS_VISIBLE | WS_VSCROLL | ES_MULTILINE | ES_READONLY | ES_AUTOVSCROLL,
        layout.panels_x, logs_y, layout.panels_w, logs_h,
        gfx->hwnd,
        (HMENU)2001,
        GetModuleHandle(NULL),
        NULL
    ...
    for (int i = 0; i < 8; i++) {
        *button_handles[i] = CreateWindowEx(0, "BUTTON", button_labels[i],
                                          ...
                                          button_start_x, layout.buttons_y, BUTTON_WIDTH, BUTTON_HEIGHT,
                                          gfx->hwnd, (HMENU)(1001 + i), GetModuleHandle(NULL), NULL);
        button_start_x += BUTTON_WIDTH + BUTTON_SPACING;
    }
