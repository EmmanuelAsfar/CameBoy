...
- HALT: CPU s’arrête jusqu’à interruption; HALT bug si IME=0 et IRQ en attente → PC peut ne pas s’incrémenter
...
- HALT: 
  - si `IME=0` et `IE&IF!=0` → `halt_bug=true`, `halted=false`, `pc` pas incrémenté (bug HALT)
...
STOP a deux comportements selon la pr�paration et le mod�le:
- DMG ou CGB sans pr�paration: STOP met le CPU en veille basse consommation (r�veil via joypad). Pour simplifier dans un cadre p�dagogique, on peut traiter ceci comme un HALT prolong�.
- CGB avec pr�paration: si `KEY1` (`0xFF4D`) a `bit0=1` (pr�paration), l�ex�cution de STOP bascule la vitesse CPU (bit7 de `KEY1` refl�te l��tat: 0=normal, 1=double). La bascule efface `bit0` et l�ex�cution continue sans entrer en veille.

D�tails `KEY1` (CGB uniquement):
- `bit7` (lecture seule): vitesse actuelle (0 = normal, 1 = double)
- `bit0` (lecture/�criture): pr�paration de bascule (�crire 1 avant STOP)
- bits 1�6: lecture � 1

Impact: la vitesse double affecte le rythme des sous-syst�mes synchronis�s au CPU (timers, APU, certaines dur�es PPU c�t� h�te). Dans ce projet, on maintient la logique � fr�quence CPU fixe et on traite le double-speed comme un facteur d�horloge global � propager au besoin dans les modules concern�s.
