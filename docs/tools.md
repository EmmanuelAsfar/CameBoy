...
- Optionnel: variables d'env
  ...
  - UtilisAes si les dossiers `tools/` ne sont pas prAsents
...
- Builder tous les exemples (si outils installAs):
```cmd
...
- `rgbfix` remplit correctement l'en-tAte (logo, checksums) pour compatibilitA.
- La GUI CameBoy affiche les logs sArie: Acrivez sur `SB`/`SC` pour tracer.
- Pour des ROMs > 32 KiB, choisissez un MBC adaptA (cf. [MBCs](https://gbdev.io/pandocs/MBCs.html)).
