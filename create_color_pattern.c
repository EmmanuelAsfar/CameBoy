...
int main() {
    FILE* f = fopen("color_pattern.gb", "wb");
    if (!f) {
        printf("Erreur: Impossible de créer color_pattern.gb\n");
        return 1;
    }
    ...
    // Title
    strcpy((char*)(header + 0x34), "COLOR TEST");
    ...
    // Code très simple - juste activer le LCD
    uint8_t rom_code[] = {
    ...
    fclose(f);
    printf("ROM de test de couleurs créée: color_pattern.gb\n");
    return 0;
}
