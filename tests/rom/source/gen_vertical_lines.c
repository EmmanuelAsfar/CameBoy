// Générateur ROM: vertical_lines – bandes verticales 3/2/1/0 répétées
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdint.h>
...
int main(void) {
    ...
    if (!rom) { fprintf(stderr, "Alloc ROM vertical_lines échouée\n"); return 1; }
    write_header(rom, "VERTICAL LINES  ");

    // Tiles: 0=vide, 1=bandes verticales 3/2/1/0/3/2/1/0
    uint8_t tiles[32];
    memset(tiles, 0x00, 16); // tile 0 (blank)
    for (int row = 0; row < 8; row++) {
        // bas = 0xAA (10101010), haut = 0xCC (11001100)
        tiles[16 + row*2 + 0] = 0xAA; // low plane bits for index 1 columns
        tiles[16 + row*2 + 1] = 0xCC; // high plane bits for index 2/3 columns
    }

    // Map = tile 1 partout (bandes sur toute la largeur)
    uint8_t map[32*18];
    for (int i = 0; i < 32*18; i++) map[i] = 1;

    // Code: LCD off, BGP, copy tiles+map, LCD on, loop
    ...
    *c++ = 0x0B;
    int8_t rel1 = (int8_t)(loop1 - (uint16_t)(p + (c - &rom[p]) + 2)); *c++ = 0x18; *c++ = (uint8_t)rel1;
    ...
    *c++ = 0x18; *c++ = 0xFE; // loop
    ...
    FILE *f = fopen("../vertical_lines.gb", "wb");
    if (!f) { fprintf(stderr, "Créer vertical_lines.gb échoué\n"); free(rom); return 1; }
